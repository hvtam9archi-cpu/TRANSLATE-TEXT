...
namespace TranslateText.Core
{
    ...
    {
        public static string ApplyCase(string text, TextCaseOption option)
        {
            ...
            switch (option)
            {
                ...
                case TextCaseOption.UpperCase:
                    return text.ToUpper();
                case TextCaseOption.SentenceCase:
                    if (text.Length > 0)
                    {
                        var firstChar = text[0].ToString().ToUpper();
                        return firstChar + text.Substring(1).ToLower();
                    }
                    return text;
        ...
        public static TextCaseOption DetectOriginalCase(string originalText)
        {
            ...
            return TextCaseOption.None;
        }

        public static string ApplyCaseSafe(string text, TextCaseOption option)
        {
            if (string.IsNullOrEmpty(text) || option == TextCaseOption.None) return text;
            var mask = FormatProtector.MaskText(text);
            string processed = ApplyCase(mask.MaskedText, option);
            return FormatProtector.UnmaskText(processed, mask.Codes);
        }
    }

    /// <summary>
    /// Bảo vệ các mã định dạng đặc biệt của AutoCAD (MText codes, Unicode escapes, etc.)
    /// bằng cách thay thế chúng bằng placeholder trước khi dịch, sau đó khôi phục lại.
    /// </summary>
    ...
    /// <summary>
    /// Bộ chuyển đổi mã ký tự tiếng Việt (Unicode ↔ VNI ↔ TCVN3)
    /// Hỗ trợ nhận diện tự động loại encoding dựa trên nội dung và tên font.
    /// </summary>
    public static class VnCharset
    {
        ...
        public static string Convert(string input, EncodingType source, EncodingType target, string fontName = "")
        {
            if (string.IsNullOrEmpty(input) || source == target) return input;
            string unicodeTemp = input;

            if (source == EncodingType.Auto) source = DetectEncoding(input, fontName);
        ...
        /// <summary>
        /// Nhận diện tự động loại encoding dựa trên nội dung text và tên font (nếu có).
        /// Ưu tiên: Font name → Ký tự > 255 (Unicode chắc chắn) → Scoring heuristics.
        /// </summary>
        public static EncodingType DetectEncoding(string text, string fontName = "")
        {
            if (string.IsNullOrEmpty(text)) return EncodingType.Unicode;

            // XÉT ƯU TIÊN MẠNH NHẤT TỪ TÊN FONT (Nếu người dùng truyền vào)
            if (!string.IsNullOrEmpty(fontName))
            {
                string fn = fontName.ToLower();
                if (fn.Contains(".vn") || fn.Contains("tcvn") || fn.Contains("vntime") || fn.Contains("xvn"))
                    return EncodingType.TCVN3;
                if (fn.Contains("vni-") || fn.Contains("vni_"))
                    return EncodingType.VNI;
            }
            ...
            // Do đó, nếu Text chứa BẤT KỲ ký tự nào có mã giá trị > 255 → chắc chắn Unicode.
            foreach (char c in clean)
            {
                if ((int)c > 255) return EncodingType.Unicode;
            }

            // BƯỚC 3: Nếu tất cả ký tự <= 255 (Vùng giao thoa nhạy cảm giữa TCVN3/VNI/Unicode)
            int sTCVN = 0, sVNI = 0, sUni = 0;

            foreach (char c in clean)
            {
                if (TcvnToUni.ContainsKey(c)) sTCVN++;
                if (UnicodeVietnameseChars.Contains(c)) sUni++;
            }
            ...
            // Đặc trưng 1 của TCVN3: Chữ hoa Latin lọt thỏm giữa các chữ thường
            if (Regex.IsMatch(clean, @"[a-z][ÊÈÉÌÕÒÓÔÝ]|[ÊÈÉÌÕÒÓÔÝ][a-z]"))
            {
                sTCVN += 5;
            }

            // Đặc trưng 2 của TCVN3: Các cụm từ CAD phổ biến khi gõ TCVN3
            if (Regex.IsMatch(clean, @"\b(chó|phô|tõ|sù|tù|cã|bé|râ|mê|mì|mò|kú)\b", RegexOptions.IgnoreCase))
            {
                sTCVN += 2;
            }

            // Chốt kết quả dựa trên điểm số
